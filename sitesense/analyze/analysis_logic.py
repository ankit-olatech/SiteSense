...
# in issue_site.py or a new file like tasks_analysis.py
...
def fetch_with_selenium(url):
    print("Initializing Selenium for URL:", url)
    ...
    try:
        ...
        driver.get(url)
        time.sleep(random.uniform(1.0, 2.0))  # wait to simulate real user
...
def analyze_page_synchronously(url):
    ...
    if not page_content:
        return {"error": "Failed to fetch content using Selenium."}

    print("Running analysis functions...")

    result = {
        "on_page_optimization": analyze_on_page_optimization(page_content),
        "h1_tag": analyze_h1_tag(page_content),
        "schema_validation": validate_schema(page_content),
        "ai_content_detection": detect_ai_content(page_content),
        "page_speed": analyze_page_speed(url),
        "meta_tags": check_meta_tags(page_content),
        "keyword_summary": analyze_keyword_summary(page_content),
        "anchor_tags": analyze_anchor_tags(page_content),
        "url_structure": analyze_url_structure(url),
        "robots_txt": validate_robots_txt(url),
        "xml_sitemap": validate_sitemap(url),
        "blog_optimization": analyze_blog_optimization(page_content, url),
        "detect_broken_urls": detect_broken_urls(url),
        "html_structure": analyze_html_structure(page_content),
        "da_pa_spam_score": analyze_da_pa_spam(url),
    }
